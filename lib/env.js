...
function ensureRequired() {
  ...
  // Next.js statically evaluates every route module during `next build` (the
  // "collect page data" step). Runtime-only secrets — Resend, Upstash, the
  // session secret — are NOT needed to compile the app, and some platforms
  // (e.g. Vercel) don't expose them during that phase. Hard-throwing here would
  // break the build for a variable the build itself never uses. So during the
  // build phase we downgrade to a warning; the check still fails fast at real
  // runtime boot (first request), where these secrets genuinely are required.
  const isBuildPhase =
